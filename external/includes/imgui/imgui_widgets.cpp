...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    ImGuiWindow* backup_hoveredwindow_manager = g.HoveredWindow;
    ...
    if (flatten_hovered_children)
        g.HoveredWindow = backup_hoveredwindow_manager;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImGuiNextWindowDataFlags backup_nextwindow_manager_data_flags = g.NextWindowData.Flags;
    ...
    g.NextWindowData.Flags = backup_nextwindow_manager_data_flags;
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (ImGuiWindow* popupwindow_manager = FindWindowByName(name))
        if (popupwindow_manager->WasActive)
        {
            // Always override 'AutoPosLastDirection' to not leave a chance for a past value to affect us.
            ImVec2 size_expected = CalcWindowNextAutoFitSize(popupwindow_manager);
            popupwindow_manager->AutoPosLastDirection = (flags & ImGuiComboFlags_PopupAlignLeft) ? ImGuiDir_Left : ImGuiDir_Down; // Left = "Below, Toward Left", Down = "Below, Toward Right (default)"
            ImRect r_outer = GetPopupAllowedExtentRect(popupwindow_manager);
            ImVec2 pos = FindBestWindowPosForPopupEx(bb.GetBL(), size_expected, &popupwindow_manager->AutoPosLastDirection, r_outer, bb, ImGuiPopupPositionPolicy_ComboBox);
            SetNextWindowPos(pos);
        }

    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVarX(ImGuiStyleVarwindow_managerPadding, g.Style.FramePadding.x); // Horizontally align ourselves with the framed text
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGuiWindow* drawwindow_manager = window;
    ...
    if (is_multiline)
    {
        ...
        PushStyleVar(ImGuiStyleVarwindow_managerPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
        ...
        drawwindow_manager = g.CurrentWindow; // Child window
        drawwindow_manager->DC.NavLayersActiveMaskNext |= (1 << drawwindow_manager->DC.NavLayerCurrent); // This is to ensure that EndChild() will display a navigation highlight so we can "enter" into it.
        drawwindow_manager->DC.CursorPos += style.FramePadding;
        inner_size.x -= drawwindow_manager->ScrollbarSizes.x;
    }
    else
    {
    ...
    const bool user_scroll_finish = is_multiline && state != NULL && g.ActiveId == 0 && g.ActiveIdPreviousFrame == GetWindowScrollbarID(drawwindow_manager, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline && state != NULL && g.ActiveId == GetWindowScrollbarID(drawwindow_manager, ImGuiAxis_Y);
    bool clear_active_id = false;
    bool select_all = false;

    float scroll_y = is_multiline ? drawwindow_manager->Scroll.y : FLT_MAX;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (is_multiline && state != NULL)
            state->Scroll.y = drawwindow_manager->Scroll.y;
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - drawwindow_manager->DC.CursorPos.y) : (g.FontSize * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(drawwindow_manager, ImMax(drawwindow_manager->Scroll.y - g.FontSize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(drawwindow_manager, ImMin(drawwindow_manager->Scroll.y + g.FontSize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
    ...
    ImVec2 draw_pos = is_multiline ? drawwindow_manager->DC.CursorPos : frame_bb.Min + style.FramePadding;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                draw_pos.y += (drawwindow_manager->Scroll.y - scroll_y);   // Manipulate cursor pos immediately avoid a frame of lag
                drawwindow_manager->Scroll.y = scroll_y;
            }
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        drawwindow_manager->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color);
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            drawwindow_manager->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                drawwindow_manager->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));
    ...
    if (is_multiline)
    {
        ...
        EndGroup();
        if (g.LastItemData.ID == 0 || g.LastItemData.ID != GetWindowScrollbarID(drawwindow_manager, ImGuiAxis_Y))
        {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    ImGuiWindow* picker_activewindow_manager = NULL;
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            {
                picker_activewindow_manager = g.CurrentWindow;
    ...
    // Convert back
    if (value_changed && picker_activewindow_manager == NULL)
    {
    ...
    if (picker_activewindow_manager && g.ActiveId != 0 && g.ActiveIdWindow == picker_activewindow_manager)
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    ImGuiWindow* barwindow_manager = FindWindowByName(name);
    if (barwindow_manager == NULL || barwindow_manager->BeginCount == 0)
    {
    ...
    PushStyleVar(ImGuiStyleVarwindow_managerRounding, 0.0f);
    PushStyleVar(ImGuiStyleVarwindow_managerMinSize, ImVec2(0, 0)); // Lift normal size constraint
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        ImGuiWindow* child_menuwindow_manager = (child_popup && child_popup->Window && child_popup->Window->ParentWindow == window) ? child_popup->Window : NULL;
        if (g.HoveredWindow == window && child_menuwindow_manager != NULL)
        {
            const float ref_unit = g.FontSize; // FIXME-DPI
            const float child_dir = (window->Pos.x < child_menuwindow_manager->Pos.x) ? 1.0f : -1.0f;
            const ImRect nextwindow_manager_rect = child_menuwindow_manager->Rect();
            ImVec2 ta = (g.IO.MousePos - g.IO.MouseDelta);
            ImVec2 tb = (child_dir > 0.0f) ? nextwindow_manager_rect.GetTL() : nextwindow_manager_rect.GetTR();
            ImVec2 tc = (child_dir > 0.0f) ? nextwindow_manager_rect.GetBL() : nextwindow_manager_rect.GetBR();
...
void ImGui::EndMenu()
{
    ...
    ImGuiWindow* parentwindow_manager = window->ParentWindow;  // Should always be != NULL is we passed assert.
    if (window->BeginCount == window->BeginCountPreviousFrame)
        if (g.NavMoveDir == ImGuiDir_Left && NavMoveRequestButNoResultYet())
            if (g.NavWindow && (g.NavWindow->RootWindowForNav == window) && parentwindow_manager->DC.LayoutType == ImGuiLayoutType_Vertical)
            {
...
namespace ImGui
{
    ...
    static ImU32            TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* dockedwindow_manager);
...
static ImU32   ImGui::TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* dockedwindow_manager)
{
    IM_ASSERT(dockedwindow_manager == NULL); // master branch only
    IM_UNUSED(dockedwindow_manager);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* dockedwindow_manager)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = TabBarCalcTabID(tab_bar, label, dockedwindow_manager);
