...
// Helper
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outerwindow_manager)
{
    // Adjust flags: set default sizing policy
    if ((flags & ImGuiTableFlags_SizingMask_) == 0)
        flags |= ((flags & ImGuiTableFlags_ScrollX) || (outerwindow_manager->Flags & ImGuiWindowFlags_AlwaysAutoResize)) ? ImGuiTableFlags_SizingFixedFit : ImGuiTableFlags_SizingStretchSame;
    ...
    if (outerwindow_manager->RootWindow->Flags & ImGuiWindowFlags_NoSavedSettings)
        flags |= ImGuiTableFlags_NoSavedSettings;
...
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* outerwindow_manager = GetCurrentWindow();
    if (outerwindow_manager->SkipItems) // Consistent with other tables + beneficial side effect that assert on miscalling EndTable() will be more visible.
        return false;
    ...
    // - always reserving the pooled ImGuiTable data ahead for a fully clipped table (minor IMHO). Also the 'outerwindow_manager_is_measuring_size' criteria may already be defeating this in some situations.
    // - always performing the GetOrAddByKey() O(log N) query in g.Tables.Map[].
    const bool use_childwindow_manager = (flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) != 0;
    const ImVec2 avail_size = GetContentRegionAvail();
    const ImVec2 actual_outer_size = ImTrunc(CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f), use_childwindow_manager ? ImMax(avail_size.y, 1.0f) : 0.0f));
    const ImRect outer_rect(outerwindow_manager->DC.CursorPos, outerwindow_manager->DC.CursorPos + actual_outer_size);
    const bool outerwindow_manager_is_measuring_size = (outerwindow_manager->AutoFitFramesX > 0) || (outerwindow_manager->AutoFitFramesY > 0); // Doesn't apply to AlwaysAutoResize windows!
    if (use_childwindow_manager && IsClippedEx(outer_rect, 0) && !outerwindow_manager_is_measuring_size)
    {
        ItemSize(outer_rect);
    ...
    flags = TableFixFlags(flags, outerwindow_manager);
    ...
    table->OuterWindow = table->InnerWindow = outerwindow_manager;
    ...
    if (use_childwindow_manager)
    {
        ...
        ImGuiWindowFlags childwindow_manager_flags = (flags & ImGuiTableFlags_ScrollX) ? ImGuiWindowFlags_HorizontalScrollbar : ImGuiWindowFlags_None;
        BeginChildEx(name, instance_id, outer_rect.GetSize(), ImGuiChildFlags_None, childwindow_manager_flags);
        ...
        if (table->InnerWindow->SkipItems && outerwindow_manager_is_measuring_size)
    ...
    ImGuiWindow* innerwindow_manager = table->InnerWindow;
    table->HostIndentX = innerwindow_manager->DC.Indent.x;
    table->HostClipRect = innerwindow_manager->ClipRect;
    table->HostSkipItems = innerwindow_manager->SkipItems;
    temp_data->HostBackupWorkRect = innerwindow_manager->WorkRect;
    temp_data->HostBackupParentWorkRect = innerwindow_manager->ParentWorkRect;
    temp_data->HostBackupColumnsOffset = outerwindow_manager->DC.ColumnsOffset;
    temp_data->HostBackupPrevLineSize = innerwindow_manager->DC.PrevLineSize;
    temp_data->HostBackupCurrLineSize = innerwindow_manager->DC.CurrLineSize;
    temp_data->HostBackupCursorMaxPos = innerwindow_manager->DC.CursorMaxPos;
    temp_data->HostBackupItemWidth = outerwindow_manager->DC.ItemWidth;
    temp_data->HostBackupItemWidthStackSize = outerwindow_manager->DC.ItemWidthStack.Size;
    innerwindow_manager->DC.PrevLineSize = innerwindow_manager->DC.CurrLineSize = ImVec2(0.0f, 0.0f);
    ...
    if (innerwindow_manager != outerwindow_manager)
    {
        // FIXME: Because innerwindow_manager's Scrollbar doesn't know about border size, since it's not encoded in window->WindowBorderSize,
        ...
        if (flags & ImGuiTableFlags_BordersOuterV)
        {
            ...
            if (innerwindow_manager->DecoOuterSizeX2 == 0.0f)
        ...
        if (flags & ImGuiTableFlags_BordersOuterH)
        {
            ...
            if (innerwindow_manager->DecoOuterSizeY2 == 0.0f)
    ...
    table->InnerClipRect = (innerwindow_manager == outerwindow_manager) ? table->WorkRect : innerwindow_manager->ClipRect;
    table->InnerClipRect.ClipWith(table->WorkRect);     // We need this to honor inner_width
    table->InnerClipRect.ClipWithFull(table->HostClipRect);
    table->InnerClipRect.Max.y = (flags & ImGuiTableFlags_NoHostExtendY) ? ImMin(table->InnerClipRect.Max.y, innerwindow_manager->WorkRect.Max.y) : table->HostClipRect.Max.y;
    ...
    outerwindow_manager->DC.NavIsScrollPushableX = false; // Shortcut for NavUpdateCurrentWindowIsScrollPushableX();
    outerwindow_manager->DC.CurrentTableIdx = table_idx;
    if (innerwindow_manager != outerwindow_manager) // So EndChild() within the inner window can restore the table properly.
        innerwindow_manager->DC.CurrentTableIdx = table_idx;
    ...
    innerwindow_manager->SkipItems = true;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Note that scrolling tables (where innerwindow_manager != outerwindow_manager) handle Y clipped earlier in BeginTable() so IsVisibleY really only applies to non-scrolling tables.
        // FIXME-TABLE: Because InnerClipRect.Max.y is conservatively ==outerwindow_manager->ClipRect.Max.y, we never can mark columns _Above_ the scroll line as not IsVisibleY.
        // Taking advantage of LastOuterHeight would yield good results there...
        // FIXME-TABLE: Y clipping is disabled because it effectively means not submitting will reduce contents width which is fed to outerwindow_manager->DC.CursorMaxPos.x,
        // and this may be used (e.g. typically by outerwindow_manager using AlwaysAutoResize or outerwindow_manager's horizontal scrollbar, but could be something else).
        ...
        // (table->HostSkipItems is a copy of innerwindow_manager->SkipItems before we cleared it above in Part 2)
    ...
    if (table->Flags & ImGuiTableFlags_NoClip)
        table->DrawSplitter->SetCurrentChannel(innerwindow_manager->DrawList, TABLE_DRAW_CHANNEL_NOCLIP);
    else
        innerwindow_manager->DrawList->PushClipRect(innerwindow_manager->InnerClipRect.Min, innerwindow_manager->InnerClipRect.Max, false); // FIXME: use table->InnerClipRect?
}
...
void    ImGui::EndTable()
{
    ...
    ImGuiWindow* innerwindow_manager = table->InnerWindow;
    ImGuiWindow* outerwindow_manager = table->OuterWindow;
    ImGuiTableTempData* temp_data = table->TempData;
    IM_ASSERT(innerwindow_manager == g.CurrentWindow);
    IM_ASSERT(outerwindow_manager == innerwindow_manager || outerwindow_manager == innerwindow_manager->ParentWindow);
    ...
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    innerwindow_manager->DC.PrevLineSize = temp_data->HostBackupPrevLineSize;
    innerwindow_manager->DC.CurrLineSize = temp_data->HostBackupCurrLineSize;
    innerwindow_manager->DC.CursorMaxPos = temp_data->HostBackupCursorMaxPos;
    const float inner_content_max_y = table->RowPosY2;
    IM_ASSERT(table->RowPosY2 == innerwindow_manager->DC.CursorPos.y);
    if (innerwindow_manager != outerwindow_manager)
        innerwindow_manager->DC.CursorMaxPos.y = inner_content_max_y;
    ...
    // FIXME: This ideally should be done earlier, in BeginTable() SetNextWindowContentSize call, just like writing to innerwindow_manager->DC.CursorMaxPos.y,
    ...
    if (!(flags & ImGuiTableFlags_NoClip))
        innerwindow_manager->DrawList->PopClipRect();
    innerwindow_manager->ClipRect = innerwindow_manager->DrawList->_ClipRectStack.back();
    ...
    splitter->SetCurrentChannel(innerwindow_manager->DrawList, 0);
    if ((table->Flags & ImGuiTableFlags_NoClip) == 0)
        TableMergeDrawChannels(table);
    splitter->Merge(innerwindow_manager->DrawList);
    ...
    // Update scroll
    if ((table->Flags & ImGuiTableFlags_ScrollX) == 0 && innerwindow_manager != outerwindow_manager)
    {
        innerwindow_manager->Scroll.x = 0.0f;
    }
    else if (table->LastResizedColumn != -1 && table->ResizedColumn == -1 && innerwindow_manager->ScrollbarX && table->InstanceInteracted == table->InstanceCurrent)
    {
        // When releasing a column being resized, scroll to keep the resulting column in sight
        const float neighbor_width_to_keep_visible = table->MinColumnWidth + table->CellPaddingX * 2.0f;
        ImGuiTableColumn* column = &table->Columns[table->LastResizedColumn];
        if (column->MaxX < table->InnerClipRect.Min.x)
            SetScrollFromPosX(innerwindow_manager, column->MaxX - innerwindow_manager->Pos.x - neighbor_width_to_keep_visible, 1.0f);
    ...
    // Pop from id stack
    IM_ASSERT_USER_ERROR(innerwindow_manager->IDStack.back() == table_instance->TableInstanceID, "Mismatching PushID/PopID!");
    IM_ASSERT_USER_ERROR(outerwindow_manager->DC.ItemWidthStack.Size >= temp_data->HostBackupItemWidthStackSize, "Too many PopItemWidth!");
    if (table->InstanceCurrent > 0)
        PopID();
    PopID();

    // Restore window data that we modified
    const ImVec2 backup_outer_max_pos = outerwindow_manager->DC.CursorMaxPos;
    innerwindow_manager->WorkRect = temp_data->HostBackupWorkRect;
    innerwindow_manager->ParentWorkRect = temp_data->HostBackupParentWorkRect;
    innerwindow_manager->SkipItems = table->HostSkipItems;
    outerwindow_manager->DC.CursorPos = table->OuterRect.Min;
    outerwindow_manager->DC.ItemWidth = temp_data->HostBackupItemWidth;
    outerwindow_manager->DC.ItemWidthStack.Size = temp_data->HostBackupItemWidthStackSize;
    outerwindow_manager->DC.ColumnsOffset = temp_data->HostBackupColumnsOffset;
    ...
    if (innerwindow_manager != outerwindow_manager)
    {
        short backup_nav_layers_active_mask = innerwindow_manager->DC.NavLayersActiveMask;
        innerwindow_manager->DC.NavLayersActiveMask |= 1 << ImGuiNavLayer_Main; // So empty table don't appear to navigate differently.
        g.CurrentTable = NULL; // To avoid error recovery recursing
        EndChild();
        g.CurrentTable = table;
        innerwindow_manager->DC.NavLayersActiveMask = backup_nav_layers_active_mask;
    }
    else
    {
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
        ...
        outerwindow_manager->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x, table->OuterRect.Min.x + table->ColumnsAutoFitWidth);
    }
    else if (temp_data->UserOuterSize.x <= 0.0f)
    {
        ...
        const float decoration_size = table->TempData->AngledHeadersExtraWidth + ((table->Flags & ImGuiTableFlags_ScrollY) ? innerwindow_manager->ScrollbarSizes.x : 0.0f);
        outerwindow_manager->DC.IdealMaxPos.x = ImMax(outerwindow_manager->DC.IdealMaxPos.x, inner_content_max_x + decoration_size - temp_data->UserOuterSize.x);
        outerwindow_manager->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x, ImMin(table->OuterRect.Max.x, inner_content_max_x + decoration_size));
    }
    else
    {
        outerwindow_manager->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x, table->OuterRect.Max.x);
    }
    if (temp_data->UserOuterSize.y <= 0.0f)
    {
        const float decoration_size = (table->Flags & ImGuiTableFlags_ScrollX) ? innerwindow_manager->ScrollbarSizes.y : 0.0f;
        outerwindow_manager->DC.IdealMaxPos.y = ImMax(outerwindow_manager->DC.IdealMaxPos.y, inner_content_max_y + decoration_size - temp_data->UserOuterSize.y);
        outerwindow_manager->DC.CursorMaxPos.y = ImMax(backup_outer_max_pos.y, ImMin(table->OuterRect.Max.y, inner_content_max_y + decoration_size));
    }
    else
    {
        // OuterRect.Max.y may already have been pushed downward from the initial value (unless ImGuiTableFlags_NoHostExtendY is set)
        outerwindow_manager->DC.CursorMaxPos.y = ImMax(backup_outer_max_pos.y, table->OuterRect.Max.y);
    }
    ...
    IM_ASSERT(g.CurrentWindow == outerwindow_manager && g.CurrentTable == table);
    ...
    outerwindow_manager->DC.CurrentTableIdx = g.CurrentTable ? g.Tables.GetIndex(g.CurrentTable) : -1;
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    ImDrawList* inner_drawlist = innerwindow_manager->DrawList;
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        // (Without the offset, in outerwindow_manager it would be rendered behind cells, because child windows are above their
        // parent. In innerwindow_manager, it won't reach out over scrollbars. Another weird solution would be to display part
