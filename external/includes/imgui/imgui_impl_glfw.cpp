...
//  2021-08-17: *BREAKING CHANGE*: Now using glfwSetWindowFocusCallback() to calling io.AddFocusEvent(). If you called ImGui_ImplGlfw_InitXXX() with install_callbacks = false, you MUST install glfwSetWindowFocusCallback() and forward it to the backend via ImGui_ImplGlfwwindow_managerFocusCallback().
...
void ImGui_ImplGlfwwindow_managerFocusCallback(GLFWwindow* window, int focused)
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
...
void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
    ...
    bd->PrevUserCallbackWindowFocus = glfwSetWindowFocusCallback(window, ImGui_ImplGlfwwindow_managerFocusCallback);
...
void ImGui_ImplGlfw_SetCallbacksChainForAllWindows(bool chain_for_allwindow_managers)
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    bd->CallbacksChainForAllWindows = chain_for_allwindow_managers;
}
...
#ifdef EMSCRIPTEN_USE_EMBEDDED_GLFW3
        const bool iswindow_manager_focused = true;
#else
        const bool iswindow_manager_focused = glfwGetWindowAttrib(window, GLFW_FOCUSED) != 0;
#endif
        if (iswindow_manager_focused)
        {
...
void ImGui_ImplGlfw_InstallEmscriptenCallbacks(GLFWwindow*, const char* canvas_selector)
{
    ...
    emscripten_set_resize_callback(EMSCRIPTEN_EVENT_TARGETwindow_manager, bd, false, ImGui_ImplGlfw_OnCanvasSizeChange);
